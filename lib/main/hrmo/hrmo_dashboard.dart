...
class HrmoRenewalApprovalPage extends StatefulWidget {
  ...
  @override
  State<HrmoRenewalApprovalPage> createState() => _HrmoRenewalApprovalPageState();
}

class _HrmoRenewalApprovalPageState extends State<HrmoRenewalApprovalPage>
  ...
  // ── New Applications tab (borrowing_applications_version2, personnel only)
  ...
  // ── Discipline tab - UPDATED to use liabilities_version2
  String selectedDisciplineTab = 'forwarded_hrmo';
  ...
  Future<void> _loadMetrics() async {
    ...
    final newRejected = await supabase
        .from('borrowing_applications_version2')
        .select('id')
        .eq('status', 'hrmo_rejected')
        .eq('user_type', 'personnel')
        .ilike('campus', userCampus!);

    // ── Discipline metrics — UPDATED to use liabilities_version2
    final openRaw = await supabase
        .from('liabilities_version2')
        .select('id')
        .eq('penalty_status', 'forwarded_hrmo')
        .eq('user_type', 'personnel')
        .ilike('campus', userCampus!)
    ...
    setState(() {
      ...
      approvedCount = (approvedHrmo as List).length;
  ...
  // ── New Applications (borrowing_applications_version2, personnel only)
  ...
  // ── Discipline cases — UPDATED to use liabilities_version2
  Future<void> _fetchDisciplineCases() async {
    if (userCampus == null) return;
    try {
      final isOpen = selectedDisciplineTab == 'forwarded_hrmo';
          ...
          Container(
            ...
            child: TabBar(
              ...
              tabs: [
                ...
                // Tab 2 – New Applications (badge on pending count)
                Tab(
                  icon: Stack(
                ...
                // Tab 3 – Disciplinary Cases (badge on open count)
                Tab(
                  icon: Stack(
                    clipBehavior: Clip.none,
                    children: [
  ...
  Widget _buildNewApplicationsTab() {
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        padding: const EdgeInsets.all(32),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _buildNewAppTitle(), const SizedBox(height: 32),
          _buildNewAppMetricCards(), const SizedBox(height: 32),
          _buildNewAppFilterTabs(), const SizedBox(height: 24),
      ...
      children: [
        Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(
              userCampus != null
                  ? 'Campus: ${userCampus!.toUpperCase()}  •  Review new personnel applications'
                  : 'Review new personnel applications',
  ...
  Widget _buildNewAppFilterTabs() {
    return Row(children: [
      _filterTab(
        ...
        onTap: () { setState(() => selectedNewAppStatus = 'pending_application'); _fetchNewApplications(); },
      ),
      const SizedBox(width: 12),
      _filterTab(
        value: 'hrmo_approved', label: 'Approved',
        icon: Icons.check_circle_rounded, color: const Color(0xFF388E3C),
        isSelected: selectedNewAppStatus == 'hrmo_approved',
        onTap: () { setState(() => selectedNewAppStatus = 'hrmo_approved'); _fetchNewApplications(); },
      ),
      const SizedBox(width: 12),
      _filterTab(
        value: 'hrmo_rejected', label: 'Rejected',
        icon: Icons.cancel_rounded, color: const Color(0xFFD32F2F),
        isSelected: selectedNewAppStatus == 'hrmo_rejected',
        onTap: () { setState(() => selectedNewAppStatus = 'hrmo_rejected'); _fetchNewApplications(); },
      ),
    ]);
  }
  ...
  Widget _buildNewApplicationList() {
          ...
          child: Column(children: [
            ...
            Text(
              selectedNewAppStatus == 'pending_application'
                  ...
                  : selectedNewAppStatus == 'hrmo_approved'
    ...
    return Column(children: newApplications.map((app) => _newApplicationCard(app)).toList());
  }

  Widget _newApplicationCard(Map<String, dynamic> app) {
    ...
    final isApproved    = status == 'hrmo_approved';

    final Color statusColor = isPending
        ? const Color(0xFFF57C00)
        : isApproved ? const Color(0xFF388E3C) : const Color(0xFFD32F2F);
    ...
    return Container(
      ...
      child: Row(children: [
        Container(
          width: 56, height: 56,
          decoration: BoxDecoration(
              color: statusColor.withOpacity(0.1), borderRadius: BorderRadius.circular(14)),
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('ID: $idNo  •  Office: $collegeOffice  •  Control No: $controlNumber',
      ...
      children: [
        Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(
              userCampus != null
                  ? 'Campus: ${userCampus!.toUpperCase()}  •  Finalize penalty decisions for personnel'
                  : 'Finalize penalty decisions for personnel',
              style: TextStyle(fontSize: 14, color: Colors.grey[600]),
            ),
          ]),
        ]),
  ...
  Widget _buildDisciplineFilterTabs() {
    return Row(children: [
      _disciplineFilterTab(value: 'forwarded_hrmo', label: 'Open Cases', icon: Icons.pending_actions_rounded, color: const Color(0xFFD32F2F), count: openCasesCount),
      const SizedBox(width: 12),
      ...
      return Center(
        child: Padding(
          ...
          child: Column(children: [
            Icon(Icons.inbox_outlined, size: 72, color: Colors.grey[300]),
            const SizedBox(height: 16),
            Text(selectedDisciplineTab == 'forwarded_hrmo' ? 'No open discipline cases' : 'No resolved cases yet',
                style: TextStyle(fontSize: 16, color: Colors.grey[500], fontWeight: FontWeight.w500)),
          ]),
        ),
      );
  ...
  Widget _disciplineCaseCard(Map<String, dynamic> disciplineCase) {
    final isOpen      = disciplineCase['penalty_status'] == 'forwarded_hrmo';
    final penaltyRec  = _formatPenalty(disciplineCase['penalty_recommendation'] as String? ?? '');
    final finalPenalty = _formatPenalty(disciplineCase['final_penalty_status'] as String? ?? '');
    final employeeNo  = disciplineCase['employee_no']?.toString() ?? 'N/A';
    ...
    if (!isOpen && disciplineCase['final_penalty_set_at'] != null) {
      try {
        final dt = DateTime.parse(disciplineCase['final_penalty_set_at'].toString()).toLocal();
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                _chip('Personnel', const Color(0xFF7B1FA2)),
              ]),
              const SizedBox(height: 4),
              Text('Employee No: $employeeNo', style: TextStyle(fontSize: 13, color: Colors.grey[600])),
            ]),
          ),
          if (isOpen)
            ElevatedButton.icon(
        ...
        if (disciplineCase['forwarded_notes'] != null && disciplineCase['forwarded_notes'].toString().isNotEmpty) ...[
          const SizedBox(height: 8),
          Row(children: [
            Icon(Icons.notes_rounded, size: 14, color: Colors.grey[500]),
            const SizedBox(width: 6),
            Expanded(child: Text('Notes: ${disciplineCase['forwarded_notes']}',
                style: TextStyle(fontSize: 12, color: Colors.grey[500], fontStyle: FontStyle.italic))),
          ]),
        ],
        if (disciplineCase['assessment_remarks'] != null && disciplineCase['assessment_remarks'].toString().isNotEmpty) ...[
          const SizedBox(height: 4),
          Row(children: [
            Icon(Icons.rate_review_rounded, size: 14, color: Colors.grey[500]),
            const SizedBox(width: 6),
            Expanded(child: Text('Assessment: ${disciplineCase['assessment_remarks']}',
                style: TextStyle(fontSize: 12, color: Colors.grey[500], fontStyle: FontStyle.italic))),
          ]),
        ],
      ]),
  ...
  String _formatPenalty(String penalty) {
    switch (penalty) {
      case 'terminated': return 'Permanently Terminated';
    ...
    return Row(
      ...
      children: [
        Row(children: [
          ...
          const SizedBox(width: 16),
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('HRMO Renewal Approval',
                style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Color(0xFF1A1A1A))),
            Text(
              userCampus != null
                  ? 'Campus: ${userCampus!.toUpperCase()}  •  Review and certify renewal applications'
                  : 'Review and certify renewal applications',
              style: TextStyle(fontSize: 14, color: Colors.grey[600]),
            ),
          ]),
        ]),
  ...
  Widget _buildMetricCards() {
    return Row(children: [
      Expanded(child: _metricCard(label: 'Pending', count: pendingCount, icon: Icons.pending_actions_rounded,
          gradient: const LinearGradient(colors: [Color(0xFFF57C00), Color(0xFFFFB74D)]))),
      const SizedBox(width: 20),
      Expanded(child: _metricCard(label: 'Approved', count: approvedCount, icon: Icons.check_circle_rounded,
          gradient: const LinearGradient(colors: [Color(0xFF388E3C), Color(0xFF66BB6A)]))),
      const SizedBox(width: 20),
      Expanded(child: _metricCard(label: 'Rejected', count: rejectedCount, icon: Icons.cancel_rounded,
          gradient: const LinearGradient(colors: [Color(0xFFD32F2F), Color(0xFFE57373)]))),
    ]);
  }
  ...
  Widget _buildFilterTabs() {
    return Row(children: [
      _filterTab(value: 'renewal_applied', label: 'Pending', icon: Icons.pending_actions_rounded, color: const Color(0xFFF57C00),
          isSelected: selectedStatus == 'renewal_applied',
          onTap: () { setState(() => selectedStatus = 'renewal_applied'); _fetchApplications(); }),
      const SizedBox(width: 12),
      _filterTab(value: 'renewal_hrmo_approved', label: 'Approved', icon: Icons.check_circle_rounded, color: const Color(0xFF388E3C),
          isSelected: selectedStatus == 'renewal_hrmo_approved',
          onTap: () { setState(() => selectedStatus = 'renewal_hrmo_approved'); _fetchApplications(); }),
      const SizedBox(width: 12),
      _filterTab(value: 'renewal_hrmo_rejected', label: 'Rejected', icon: Icons.cancel_rounded, color: const Color(0xFFD32F2F),
          isSelected: selectedStatus == 'renewal_hrmo_rejected',
          onTap: () { setState(() => selectedStatus = 'renewal_hrmo_rejected'); _fetchApplications(); }),
    ]);
  }

  // Unified filter tab — explicit isSelected + onTap avoids hard-coding which state var to check
  Widget _filterTab({
...
// ═══════════════════════════════════════════════════════════════════
// Enum — which table the CertifyDialog writes to
// ═══════════════════════════════════════════════════════════════════
enum _AppTableSource { renewal, newApplication }

// ═══════════════════════════════════════════════════════════════════
// FINALIZE DECISION DIALOG - UPDATED to use liabilities_version2 fields
// ═══════════════════════════════════════════════════════════════════
...
class _FinalizeDecisionDialogState extends State<_FinalizeDecisionDialog> {
  ...
  Future<void> _finalize() async {
    ...
    try {
      ...
      // UPDATED: Use liabilities_version2 fields
      await supabase.from('liabilities_version2').update({
        ...
        'final_penalty_remarks': _notesController.text.trim().isEmpty ? null : _notesController.text.trim(),
  ...
  Widget build(BuildContext context) {
    ...
    final employeeNo = widget.disciplineCase['employee_no']?.toString() ?? 'N/A';
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Container(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    const SizedBox(height: 12),
                    _caseRow('Personnel', borrowerName),
                    _caseRow('Employee No', employeeNo),
                    _caseRow('Bike Number', bikeNumber),
                    if (notes != null && notes.toString().isNotEmpty) _caseRow('Forwarded Notes', notes.toString()),
                    if (assessmentRemarks != null && assessmentRemarks.toString().isNotEmpty) _caseRow('Assessment', assessmentRemarks.toString()),
                  ]),
                ),
                ...
                const SizedBox(height: 4),
                Text('This decision is final and will be applied to the personnel.',
                    style: TextStyle(fontSize: 13, color: Colors.grey[500])),
                const SizedBox(height: 12),
                _penaltyOption(value: 'terminated', label: 'Permanently Terminated',
                    description: 'Personnel is permanently banned from the PedalHub bike borrowing program.',
                ...
                _penaltyOption(value: 'suspended_1_semester', label: 'Suspended for 1 Semester',
                    description: 'Personnel cannot borrow a bike for the next full semester.',
...
// ═══════════════════════════════════════════════════════════════════
// CERTIFY DIALOG
// Handles both renewal_applications and borrowing_applications_version2
// ═══════════════════════════════════════════════════════════════════
...
class _CertifyDialogState extends State<_CertifyDialog> {
  ...
  Future<void> _pickImage() async {
    ...
    if (result != null && result.files.isNotEmpty) {
      ...
      if (bytes != null) {
        setState(() { _uploadedImageBytes = bytes; _uploadedFileName = fileName; _hasSignature = true; });
      }
    }
  }
            ...
            child: SingleChildScrollView(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Disciplinary Status',
                    style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Color(0xFF1A1A1A))),
                const SizedBox(height: 4),
                Text('Select the applicable disciplinary status for this applicant:',
                    style: TextStyle(fontSize: 13, color: Colors.grey[600])),
                const SizedBox(height: 14),
                _disciplinaryOption(
                    value: false,
                ...
                const SizedBox(height: 20),
                const Text('HRMO Signature *',
                    style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Color(0xFF1A1A1A))),
                const SizedBox(height: 4),
                Text('Draw or upload your signature to certify this application:',
                    style: TextStyle(fontSize: 13, color: Colors.grey[600])),
                const SizedBox(height: 14),
                Row(
                    ...
                    Column(children: [
                      IconButton(
                        ...
                        onPressed: () {
                          ...
                          setState(() { _uploadedImageBytes = null; _hasSignature = false; });
                        },
                      ),
                      IconButton(
                        ...
                        onPressed: () {
                          setState(() { _isDrawMode = !_isDrawMode; _uploadedImageBytes = null; _hasSignature = _signatureController.isNotEmpty; });
                        },
                      ),
                    ]),
                  ],
